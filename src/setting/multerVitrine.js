...
const typeStorageUpload = {
    servidorImagens: multerS3({
        ...
        key: (req, file, cb) => {
            ...
            // const nomeFinal = `Vitrine/vitrine-${req.sessao_negocio}.${extensao.substring(6, extensao.length)}`
            const nomeFinal = `vitrine/${req.sessao_negocio}${Date.now()}.${extensao.substring(6, extensao.length)}`
            cb(null, nomeFinal)
        }
    })
}
